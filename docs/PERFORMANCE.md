...
| 10 | **Reversed-Z depth buffer.** `WebGPURenderer({ reversedDepthBuffer: true })` (default on; `?norevz` to disable). At real scale the near:far ratio is ~1:30, so plain float32 depth resolves only ~1 m near the surface and the apron-overlap + skirt geometry z-fought into thin seam lines at every leaf/cube-face boundary. Reversed-Z spends float32's precision near the camera and clears the seams; zero shader cost. Diagnosed with `scripts/shoot.mjs` (headless screenshots): seams were prominent under WebGPU, faint under WebGL2 — a depth-precision tell, not geometry. | no boundary seam z-fighting |
